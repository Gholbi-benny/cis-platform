    ...
    <div className="w-full max-w-6xl mx-auto text-center">
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-blue-500/50 bg-blue-500/10 p-6 text-sm text-blue-100">Aucun projet trouvé.</div>
        ) : (
            ...
            <div className="bg-blue-600 border border-blue-500 rounded-3xl p-6 max-w-md w-full">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={newProjectName} onChange={(e) => setNewProjectName(e.target.value)} className="w-full rounded-2xl border border-gray-500 bg-gray-700 px-3 py-2 text-white" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-200 mb-2">Description</label>
                  <textarea value={newProjectDescription} onChange={(e) => setNewProjectDescription(e.target.value)} className="w-full rounded-2xl border border-gray-500 bg-gray-700 px-3 py-2 text-white h-24" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-200 mb-2">Date d'échéance</label>
                  <input type="date" value={newProjectEndDate} onChange={(e) => setNewProjectEndDate(e.target.value)} className="w-full rounded-2xl border border-gray-500 bg-gray-700 px-3 py-2 text-white" />
                </div>
                {createError && <p className="text-sm text-red-300">{createError}</p>}
              </div>
              <div className="mt-6 flex space-x-2">
                <button onClick={handleCreateProject} className="rounded-2xl bg-sky-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-sky-700">Créer</button>
                <button onClick={() => { setShowForm(false); setCreateError(null); }} className="rounded-2xl bg-slate-800 px-4 py-2 text-sm font-medium text-slate-200 transition hover:bg-slate-700">Annuler</button>
              </div>
            </div>
          </div>
        )}
